...
function update_status(){
    ...
    // Minimales Fehlerhandling
   	if (!$mysqli->query($sql_query)) {
   	    echo $sql_query." ".$mysqli->error."<hr>";
    }
	foreach ($__balanceBot_coins as $key => $array_coin){
...
foreach ($array_coins_sorted_by_deviation as $key => $nothing){ // $nothing weil ich nur den $key brauche, nicht das Subarray.
	...
	// Abschnitt index.php "Change 25.01.2019" Nr. 1 Anfang 
	// Ich mache das um nur einen BUY oder einen SELL je Durchlauf durchzuführen um das Schwingen zu unterbinden.
	$create_order = "";
	if ($__balanceBot_coins[$key]['deviation_percentage'] - $__balanceBot_coins[$key]['tolerance_factor']/100 > $__balanceBot_basecurrency['deviation_percentage']){
		$create_order = "SELL";
	} else if ($__balanceBot_coins[$key]['deviation_percentage'] + $__balanceBot_coins[$key]['tolerance_factor']/100 < $__balanceBot_basecurrency['deviation_percentage']){
		$create_order = "BUY";
	} else {
		$create_order = "do nothing";
	}
	$message = "BALANCE: ".$__balanceBot_coins[$key]['name']." 
		".round($__balanceBot_coins[$key]['deviation_percentage'],3)."-".($__balanceBot_coins[$key]['tolerance_factor']/100)." 
		= ".round($__balanceBot_coins[$key]['deviation_percentage']-$__balanceBot_coins[$key]['tolerance_factor']/100,3)." 
		< ".round($__balanceBot_basecurrency['deviation_percentage'],3)." 
		< ".round($__balanceBot_coins[$key]['deviation_percentage']+$__balanceBot_coins[$key]['tolerance_factor']/100,3)." 
		= ".round($__balanceBot_coins[$key]['deviation_percentage'],3)."+".($__balanceBot_coins[$key]['tolerance_factor']/100)." 
		=> $create_order";
	update_messages($message); // Message in DB schreiben und ausgeben.
	// Abschnitt index.php "Change 25.01.2019" Nr. 1 Ende 

	// Abschnitt index.php "Change 25.01.2019" Nr. 2 Anfang 
	...
	// Abschnitt index.php "Change 25.01.2019" Nr. 1 Ende 
		...
		foreach ($array_binance_openOrders[$__balanceBot_coins[$key]['name']] as $array_openOrder){
			if ($array_openOrder['side'] == "BUY" AND $already_order_deleted == 0){
				...
				$message = " ... Order ".$__balanceBot_coins[$key]['name'].$__balanceBot_basecurrency['name']." zum Löschen gefunden!  ";
		...
		if (!$already_order_deleted){
			...
			// "Update 25.01.2019" sollte meine Wunsch-Amount mehr sein als ich verfügbare Coins habe ...
			// ich aber immernoch mehr freie Coins als die Minimalmenge habe, dann verkaufe ich einfach mal alles
			# Auskommentiert, da das eigentlich dem Grundprinzip des Bots wiederspricht wenn der Fall eintritt.
	...
	// Abschnitt index.php "Change 25.01.2019" Nr. 3 Anfang 
	...
	// 		update_messages($message); // Message in DB schreiben und ausgeben.
	if ($create_order == "BUY"){
	// Abschnitt index.php "Change 25.01.2019" Nr. 3 Ende 
		...
		if (!$already_order_deleted){
			...
			// "Update 25.01.2019" sollte meine Wunsch-Amount mehr sein als ich verfügbare Basiswährung habe ...
			// ich aber immernoch mehr freie Coins als die Minimalmenge habe, dann verkaufe ich einfach mal alles
			# Auskommentiert, da das eigentlich dem Grundprinzip des Bots wiederspricht wenn der Fall eintritt.
...
foreach ($__balanceBot_coins as $key => $array_coin){
	...
	foreach ($array_binance_openOrders[$array_coin['name']] as $array_openOrder){
		...
		if ($array_openOrder['ageInHours'] > $__max_orderAge){
			...
			$message = " Veraltete Order ".$array_coin['name'].$__balanceBot_basecurrency['name']." zum Löschen gefunden!  ";
